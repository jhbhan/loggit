...
export default function AddQuestion() {
    ...
    const selectedQuestion = useAppSelector(
        getQuestionForId(Number(questionId))
    );
    const [questionText, setQuestionText] = useState(
        selectedQuestion?.text ?? ''
    );
    const [questionType, setQuestionType] = useState<QuestionFormat>(
        selectedQuestion?.format ?? QuestionFormat.Text
    );
    const [minimum, setMinimum] = useState<number | null>(
        selectedQuestion?.ratingMin ?? null
    );
    const [maximum, setMaximum] = useState<number | null>(
        selectedQuestion?.ratingMax ?? null
    );
    const [multipleChoiceOptions, setMultipleChoiceOptions] = useState<
        string[]
    >(selectedQuestion?.options ?? []);
    const [multipleChoiceQuestionText, setMultipleChoiceQuestionText] =
        useState<string>('');
                ...
                {questionType === QuestionFormat.Number && (
                    <>
                        <ThemedView>
                            ...
                            <NumberInput
                                numberValue={minimum}
                                onNumberChange={(val: number | null) =>
                                    setMinimum(val)
                                }
                            />
                        </ThemedView>
